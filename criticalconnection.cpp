...
//code by CasioFx-69
void dfs(vector<int>adj[],int parent,vector<int>&visited,vector<int>&tsf,vector<int>&lsf,int &count,int src){
    visited[src]=1;
    tsf[src]=count;
    lsf[src]=count;
    count++;
    for(auto it:adj[src]){
        if(it==parent){
            ...
            lsf[src] = min(lsf[src],tsf[it]);
        }
        else{
            dfs(adj,src,visited,tsf,lsf,count,it);
            lsf[src] = min(lsf[src],lsf[it]);
            if(tsf[src]<lsf[it]){
                cout<<src<<" "<<it<<endl;
            }
        }
...
int main(){
    ...
    int count=1;
    int src=0;
    dfs(adj,-1,visited,tsf,lsf,count,src);
     return 0;
}